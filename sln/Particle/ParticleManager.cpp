...
ParticleManager::ParticleManager(const std::wstring& texPath)
{
	Initialize(DxBase::GetInstance()->GetDevice(), texPath);
}

void ParticleManager::Initialize(ID3D12Device* device, const std::wstring& texPath)
{
	// nullptrチェック
	assert(device);
...
void ParticleManager::LoadTexture(const std::wstring& path)
{
	// WICテクスチャのロード

...
#pragma region 自機と敵弾の衝突判定
	...
	XMFLOAT3 PlayerPosData = Player::GetPlayerPosMemory();//自機
	XMFLOAT3 EnemyBulPosData = EnemyBullet::GetEnemyBulPosMemory();//敵弾
	float PlayerRad = 5.f;//自機の判定球の半径
	float EnemyBulRad = 5.f;//敵弾の判定球の半径
	float EnemyBulPlayerDistance = 0.f;//posAposBの距離
	float PlayerEnemyBulRad = 0.f;//半径1＋半径2の二乗
...
#pragma region 自弾と雑魚敵の衝突判定
	...
	float PlayerBulSmallEnemyDistance = 0.f;//posAposBの距離
	float PlayerBulSmallEnemyRad = 0.f;//半径1＋半径2の二乗
